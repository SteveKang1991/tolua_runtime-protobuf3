...
LUALIB_API int luaopen_luapb_io(lua_State *L) {
    luaL_Reg libs[] = {
...
LUALIB_API int luaopen_luapb_conv(lua_State *L) {
    luaL_Reg libs[] = {
...
#undef  ENTRY
    ...
    //luaL_newlib(L, libs);
	luaL_register(L, "luapb.conv", libs);
    return 1;
}
...
LUALIB_API int luaopen_luapb_buffer(lua_State *L) {
    luaL_Reg libs[] = {
...
#undef  ENTRY
    ...
    if (luaL_newmetatable(L, PB_BUFFER)) {
		...
		luaL_register(L, "luapb.buffer", libs);
        lua_pushvalue(L, -1);
...
LUALIB_API int luaopen_luapb_slice(lua_State *L) {
    luaL_Reg libs[] = {
...
#undef  ENTRY
    ...
    if (luaL_newmetatable(L, PB_SLICE)) {
        //luaL_setfuncs(L, libs, 0);
		luaL_register(L, "luapb.slice", libs);
        lua_pushvalue(L, -1);
        lua_setfield(L, -2, "__index");
        lua_createtable(L, 0, 1);
